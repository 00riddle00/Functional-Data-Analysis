...
particularly useful in neuroscience, where signals such as EEG are mostly continuous.
...
\begin{itemize}[
  ...
  \item To identify dominant modes of variation among subjects through Principal Component Analysis
    (PCA).
...
adulthood, self-reports of ADHD symptoms. This project applies Functional Data Analysis methods to
EEG data from 62 adults recorded during a Flanker task at electrode channel FC1. Data source:
...
objects, in order to remove measurement noise. A penalized B-spline basis expansion was employed for
...
after $0.4\,\mathrm{s}$. This indicates that between-subject variability is substantially larger in
...
concentrated in the time region $0.5$--$0.7\,\mathrm{s}$ on both axes. This confirms that the late
post-stimulus response constitutes the dominant source of between-subject variability, whereas the
pre-stimulus baseline shows near-zero covariance, indicating consistent behaviour across subjects
during that period.
...
variation among subjects. Here are the following harmonics:
...
Massive Unsupervised Outlier Detection (MUOD). Four subjects --- sub-003, sub-040, sub-043, and
sub-071 --- were identified as outliers by the HDR boxplot at coverage level $\alpha = (0.05,
0.50)$. All four exhibit substantial negative deflections in the post-stimulus window: sub-071
(purple) shows the most extreme deviation, reaching approximately $-50~\mu$V after
$0.6\,\mathrm{s}$, while sub-040 (green) and sub-043 (blue) both descend to about $-20~\mu$V in the
same time region. Sub-003 (red) shows a milder but still atypical negative pattern. These subjects
are classified as magnitude outliers, as their waveforms have a similar overall shape to the rest of
the sample but are shifted significantly away from the group mean.
...
The objective of this part of the project is to determine whether the functional EEG response curves
during the Flanker task differ significantly between individuals with ADHD symptoms and those
without. The ADHD classification is based on the World Health Organization Adult ADHD Self-Report
Scale (ASRS-V1.1), included in the original dataset. Of the 62 subjects analyzed in the previous
sections, 12 were classified as having ADHD symptoms, 39 as not having ADHD symptoms, and 11 had
missing classifications and were excluded from this analysis. The hypotheses tested are formulated
as:
...
applied a regression model which is defined as:
...
testing. The female effect $\beta_2(t)$ is significant ($p = 0.016$), suggesting that gender does
have a measurable impact on the EEG response shape at channel FC1. The model achieves an adjusted
$R^2 = 0.108$, with approximately $11\%$ of the deviance explained.
...
the post-stimulus window, especially between $0.3$ and $0.6\,\mathrm{s}$. This indicates that female
...
Flanker cognitive task at electrode channel FC1, using 62 young adults from the publicly available
Cognitive Electrophysiology in Socioeconomic Context dataset. In the smoothing step, raw discrete
EEG observations were converted into smooth functional objects using a penalized B-spline basis
expansion with 20 cubic basis functions. The exploratory data analysis revealed that between-subject
variability is concentrated in the post-stimulus window ($0.4$--$0.8\,\mathrm{s}$), with the first
three functional principal components explaining $86.6\%$ of the total variance. Outlier detection
methods consistently identified a small number of subjects classified as magnitude outliers.

The hypothesis testing section investigated whether the mean EEG curves differ between subjects
...
$\alpha = 0.05$, indicating no statistically significant global differences between any of the
groups.
...
Overall, the project demonstrates how FDA methods can be applied to electrophysiological data to
