% vim: set ft=tex tw=100 nu ai et ts=2 sts=2 sw=2:
...
\jmlrheading{Projects Spring \textquotesingle{}26}{2026}{1--14}{May 2026}{June 2026}
...
\begin{abstract}%   <- trailing '%' for backward compatibility of .sty file
  ...
  depth-based outlier detection methods. Hypothesis testing revealed no statistically significant
  differences between groups defined by ADHD status, gender, or socioeconomic status. A
  function-on-scalar regression model further identified a significant effect of gender on the EEG
  response shape, with female subjects exhibiting systematically lower amplitudes in the
...
\begin{itemize}[
  ...
  \item To test for statistically significant differences in EEG response curves between groups
    defined by ADHD status, gender, and socioeconomic status.
  \item To model the relationship between subject characteristics and EEG response curves using
...
All analyses were carried out in R, using the \texttt{fda}, \texttt{fda.usc}, and \texttt{refund}
packages, with Python equivalents implemented natively using \texttt{scikit-fda}, \texttt{numpy},
and \texttt{scipy}.

The effect of the smoothing procedure is illustrated in Figure~\ref{fig:smoothing}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{02_before_after_smoothing.pdf}
  \caption{Observed EEG curves before and after B-spline smoothing.}
  \label{fig:smoothing}
\end{figure}
...
majority of curves, though several subjects clearly deviate beyond it. The sample mean function and
pointwise standard deviation are shown in Figure~\ref{fig:mean_sd}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{02_mean_sd.pdf}
  \caption{Sample mean function and pointwise standard deviation.}
  \label{fig:mean_sd}
\end{figure}
...
subjects during that period. The covariance structure is visualized in Figure~\ref{fig:covariance}.
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{0.48\textwidth}
    ...
    \label{fig:covariance_perspective}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.48\textwidth}
    \centering
  ...
  \end{subfigure}
  \caption{Perspective and image visualizations of the sample covariance function.}
...
functional data can be effectively summarized by a low-dimensional representation. The first four
functional principal component harmonics are displayed in Figure~\ref{fig:pca_harmonics}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{13_pca_harmonics.pdf}
  \caption{First four functional principal component harmonics.}
  \label{fig:pca_harmonics}
\end{figure}
...
results. The resulting functional boxplot is shown in Figure~\ref{fig:mbd_boxplot}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{21_fbplot_MBD.pdf}
  \caption{Functional boxplot based on Modified Band Depth (MBD).}
  \label{fig:mbd_boxplot}
\end{figure}
...
are shifted significantly away from the group mean. The four identified outlier subjects were
retained in the subsequent hypothesis testing and regression analyses to preserve sample size. The
detected magnitude outliers are illustrated in Figure~\ref{fig:hdr_boxplot}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{32_hdr_functional_005.pdf}
  \caption{Highest Density Region (HDR) functional boxplot.}
  \label{fig:hdr_boxplot}
\end{figure}
...
increased variability in the post-stimulus window. The grouped EEG response curves are presented in
Figure~\ref{fig:group_comparison}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{HT_01_group_comparison.pdf}
  \caption{EEG response curves grouped by ADHD classification.}
...
but with a smaller amplitude in the post-stimulus window, reaching approximately $+4~\mu$V. The
group mean functions are compared in Figure~\ref{fig:mean_comparison}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{HT_02_mean_comparison.pdf}
  \caption{Mean EEG response curves for ADHD and non-ADHD groups.}
...
The results are summarized in Table~\ref{tab:hypothesis_tests}.
...
To further investigate the relationship between EEG response curves and subject characteristics, a
function-on-scalar regression model was fitted. The regression was performed on the 51 subjects with
known ADHD classification, excluding the 11 subjects with missing ADHD status. The model is defined
as:
...
The terms used in the regression model are described in Table~\ref{tab:model_terms}.
...
0.108$, with approximately $11\%$ of the deviance explained. A detailed summary of the fitted
regression model is provided in Table~\ref{tab:model_summary}.
...
subjects exhibit systematically lower amplitudes. The estimated functional regression coefficients
are shown in Figure~\ref{fig:coefficients}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{REG_02_individual_coefficients.pdf}
  \caption{Estimated functional regression coefficients.}
...
driven by ADHD and gender but not other factors not included in the model. A comparison of the
observed and fitted EEG curves is presented in Figure~\ref{fig:fitted_vs_observed}.

\begin{figure}[ht]
  \centering
  \includegraphics[scale=0.5]{REG_03_fitted_vs_observed.pdf}
  \caption{Observed and fitted EEG response curves.}
...
% -----------------------------------------------------------------------------
\newpage
% References (used literature, scientific papers, and other sources) are stored in a bibliography
% file (with .bib extension). References used in this work (cited, paraphrased, or otherwise
% mentioned) are given in alphabetical order. The descriptions are not transliterated.
\bibliography{bibliography}

% -----------------------------------------------------------------------------
\newpage
% Appendices contain supplementary material supporting the main report. This may include
% implementation details, additional figures/tables, reproducibility information, or author-created
% resources. Appendices should be referenced from the main text when appropriate. Appendices are
% typically named and numbered for clarity and referenceability.
\appendix

% -----------------------------------------------------------------------------
\section{Code Availability}

The complete source code used for this project, including the R and Python implementations, data
preprocessing scripts, statistical analyses, and LaTeX report source files, is publicly available
at:

\vspace{0.8\baselineskip}

\url{https://github.com/00riddle00/Functional-Data-Analysis}

\vspace{0.8\baselineskip}

Some FDA procedures were implemented primarily in R due to the greater maturity of FDA-related
libraries in the R ecosystem.
